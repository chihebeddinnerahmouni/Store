...
interface EnhancedTableProps {
  ...
  onRequestSort: (
    ...
    property: keyof IInvDetails_achats
  ) => void;
...
function EnhancedTableHead(props: EnhancedTableProps) {
  ...
  const createSortHandler =
    (property: string) => (event: React.MouseEvent<unknown>) => {
      onRequestSort(event, property as keyof IInvDetails_achats);
    };
...
export default function EnhancedTable({
  ...
  rows: IInvDetails_achats[];
  columns: (keyof IInvDetails_achats_Table)[];
}) {
  const [order, setOrder] = React.useState<Order>("asc");
  const [orderBy, setOrderBy] = React.useState<keyof IInvDetails_achats>("id");
...
const Colval = (column: string, row: IInvDetails_achats) => {
  switch (column) {
    case "référence":
      return (
        <span className="text-blue-500 border-2 border-blue-500 px-1 rounded-[5px]">
          {row[column as keyof IInvDetails_achats_Table]}
        </span>
      );
    default:
      return <p>{row[column as keyof IInvDetails_achats_Table]}</p>;
  }
};
