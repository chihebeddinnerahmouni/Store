...
function EnhancedTableHead(props: EnhancedTableProps) {
  ...
  const createSortHandler =
    (property: string) => (event: React.MouseEvent<unknown>) => {
      onRequestSort(event, property as keyof IBestSell);
    };
...
export default function EnhancedTable({
  ...
  rows: IBestSell[];
  columns: (keyof IBestSellTable)[];
}) {
  const [order, setOrder] = React.useState<Order>("asc");
  const [orderBy, setOrderBy] =
    React.useState<keyof IBestSell>("id");
...
const Colval = (column: string, row: IBestSell) => {
  switch (column) {
    case "code":
      return (
        <span className="text-blue-500 border-2 border-blue-500 px-1 rounded-[5px]">
          {row[column as keyof IBestSellTable]}
        </span>
      );
    default:
      return <p>{row[column as keyof IBestSellTable]}</p>;
  }
};
