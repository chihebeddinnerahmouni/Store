...
function EnhancedTableHead(props: EnhancedTableProps) {
  ...
  const createSortHandler =
    (property: string) => (event: React.MouseEvent<unknown>) => {
      onRequestSort(event, property as keyof IProduit);
    };
...
export default function EnhancedTable({
  ...
  const [orderBy, setOrderBy] = React.useState<keyof IProduit>("id");
...
const Colval = (column: string, row: IProduit) => {
  switch (column) {
    case "code":
      return (
        <span className="text-blue-500 border-2 border-blue-500 px-1 rounded-[5px]">
          {row[column as keyof IProduitTable]}
        </span>
      );
    default:
      return <p>{row[column as keyof IProduitTable]}</p>;
  }
};
