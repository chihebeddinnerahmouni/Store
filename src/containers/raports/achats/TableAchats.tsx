...
function EnhancedTableHead(props: EnhancedTableProps) {
  ...
  const createSortHandler =
    (property: string) => (event: React.MouseEvent<unknown>) => {
      onRequestSort(event, property as keyof IAchatReport);
    };
...
export default function EnhancedTable({
  ...
  const [orderBy, setOrderBy] =
    React.useState<keyof IAchatReport>("id");
...
const Colval = (column: string, row: IAchatReport) => {
  switch (column) {
    case "référence":
      return (
        <span className="text-blue-500 border-2 border-blue-500 px-1 rounded-[5px]">
          {row[column as keyof IAchatReportTable]}
        </span>
      );
    default:
      return <p>{row[column as keyof IAchatReportTable]}</p>;
  }
};
