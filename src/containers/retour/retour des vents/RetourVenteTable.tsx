...
const renderColumnContent = (column: string, row: IRetourvents) => {
  ...
  switch (column) {
    case "reference":
      return <p className="text-blue-500">{value}</p>;
    ...
    case "status":
      switch (value) {
        case "Complété":
          return (
            <p className="text-green-500 border-2 border-green-500 px-1 rounded-[5px]">
              {value}
            </p>
          );
        case "En cours":
          return (
            <p className="text-red-500 border-2 border-red-500 px-1 rounded-[5px]">
              {value}
            </p>
          );
        default:
