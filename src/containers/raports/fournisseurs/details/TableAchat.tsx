...
interface EnhancedTableProps {
  ...
  onRequestSort: (
    ...
    property: keyof IProviderDetails
  ) => void;
...
function EnhancedTableHead(props: EnhancedTableProps) {
  ...
  const createSortHandler =
    (property: string) => (event: React.MouseEvent<unknown>) => {
      onRequestSort(event, property as keyof IProviderDetails);
    };
...
export default function EnhancedTable({
  ...
  rows: IProviderDetails[];
  columns: (keyof IProviderDetailsTable)[];
}) {
  const [order, setOrder] = React.useState<Order>("asc");
  const [orderBy, setOrderBy] = React.useState<keyof IProviderDetails>("id");
...
const Colval = (column: string, row: IProviderDetails) => {
  switch (column) {
    case "référence":
      return (
        <span className="text-blue-500 border-2 border-blue-500 px-1 rounded-[5px]">
          {row[column as keyof IProviderDetailsTable]}
        </span>
      );
    default:
      return <p>{row[column as keyof IProviderDetailsTable]}</p>;
  }
};
