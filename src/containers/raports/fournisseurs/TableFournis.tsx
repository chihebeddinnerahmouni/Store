...
function EnhancedTableHead(props: EnhancedTableProps) {
  ...
  const createSortHandler =
    (property: string) => (event: React.MouseEvent<unknown>) => {
      onRequestSort(event, property as keyof IProvider);
    };
...
export default function EnhancedTable({
  ...
  const [orderBy, setOrderBy] = React.useState<keyof IProvider>("id");
                  ...
                  >
                    ...
                    >
                      ...
                      <ViewButton
                        active={true}
                        onClick={() =>
                          window.open(`/rapports/fournisseurs/${row.id}`, "_blank")
                        }
                      />
                    </TableCell>
                  </TableRow>
...
const Colval = (column: string, row: IProvider) => {
  switch (column) {
    case "référence":
      return (
        <span className="text-blue-500 border-2 border-blue-500 px-1 rounded-[5px]">
          {row[column as keyof IProviderTable]}
        </span>
      );
    default:
      return <p>{row[column as keyof IProviderTable]}</p>;
  }
};
