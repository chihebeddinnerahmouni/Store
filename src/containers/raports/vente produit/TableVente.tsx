...
function EnhancedTableHead(props: EnhancedTableProps) {
  ...
  const createSortHandler =
    (property: string) => (event: React.MouseEvent<unknown>) => {
      onRequestSort(event, property as keyof IProductVente);
    };
...
export default function EnhancedTable({
  ...
  const [orderBy, setOrderBy] =
    React.useState<keyof IProductVente>("id");
...
const Colval = (column: string, row: IProductVente) => {
  switch (column) {
    case "référence":
      return (
        <span className="text-blue-500 border-2 border-blue-500 px-1 rounded-[5px]">
          {row[column as keyof IProductVenteTable]}
        </span>
      );
    default:
      return <p>{row[column as keyof IProductVenteTable]}</p>;
  }
};
